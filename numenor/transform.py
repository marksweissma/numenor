...
from _tools.utils import as_factory, call_from_attribute_or_callable
...
def include_by_key(df, inclusions=None, **kwargs):
    keys = [i for i in df
            if i in inclusions] if inclusions is not None else list(df)
    return keys
...
def include_by_key(df, use_head=True, **kwargs):
    if use_head:
        df = df.head()
    keys = list(df.select_dtypes(**kwargs))
    return keys

@variants.primary
def exclude(variant, df, inclusions, **kwargs):
    return getattr(exclude, variant)(df, inclusions, **kwargs)

@exclude.variant('by_key')
def exclude_by_key(df, inclusions, exclusions=None, **kwargs):
        keys = [i for i in inclusions if i not in exclusions]
        return keys
...
class Select:
    ...
    def __call__(self, df):
        includes = list(itertools.chain(*include(key, df, **value) for key, value in self.include.items()))
        if self.exclude:
            includes = itertools.chain(*include(key, df, **value) for key, value in self.include.items())
...
class TimeFeaturesProjectUnitCircle(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        ...
        for start, stop in self.deltas:
            X[f'{start}__{stop}_delta'] = (X[stop] -
                                           X[start]) / pd.Timedelta('1D')
